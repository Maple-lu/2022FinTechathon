...
class Company_EAdmin(admin.ModelAdmin):
    list_display = ["company", "name","date", "Greenhouse_gas_emissions"]  #显示的特征
    search_fields = ["name", "company"]  #搜索的字段
    list_filter = ["name", "add_time", "company__name","date"] #过滤

class Company_GAdmin(admin.ModelAdmin):
    list_display = ["company", "name","date", "value"] #显示的特征
    search_fields = ["name", "company"] #搜索的字段
    list_filter = ["name", "add_time", "company__name","date"] #过滤

class Company_SAdmin(admin.ModelAdmin):
    list_display = ["company", "name", "date","donate"] #显示的特征
    search_fields = ["name", "company"] #搜索的字段
    list_filter = ["name", "add_time", "company__name","date"] #过滤
